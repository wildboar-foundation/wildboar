...
def if_not_exists(*args):
    def decorator(f):
        @functools.wraps(f)
        def wrap(basepath, force=False):
...
def mk_light_dark(*args, figure=None, **kwargs):
    def decorator(f):
        @functools.wraps(f)
        def wrap(basepath, force=False):
            path = Path(basepath).joinpath(*args)
            if exists(path) and not force:
                return
            path.parent.mkdir(parents=True, exist_ok=True)
            for fig, ax in yield_and_save_plot(path, figure=figure, **kwargs):
                f(fig, ax)
...
def yield_and_save_plot(path, transparent=True, figure=None, **kwargs):
    ...
    plt.rcParams["image.cmap"] = "wildboar"
