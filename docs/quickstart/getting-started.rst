...
- Supervised learning, in which the data series are labeled with additional
  information. The additional information can be either numerical or nominal
   ...
   - In classification the time series belong to one of two or more labels and
     the goal is to learn a function that can label unlabeled time series.

   - In regression the time series are labeled with a numerical attribute and
     the task is to assign a new numerical value to an unlabeled time series.
