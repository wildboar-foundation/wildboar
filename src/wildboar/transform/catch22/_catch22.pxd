...
cdef double transition_matrix_3ac_sumdiagcov(const double *x, double *ac, Py_ssize_t n) nogil
...
cdef double local_mean_tauresrat(const double *x, double *ac, Py_ssize_t n, Py_ssize_t lag) nogil

cdef double periodicity_wang_th0_01(const double *x, int length) nogil

cdef double embed2_dist_tau_d_expfit_meandiff(const double *x, double *ac, int length) nogil

cdef double auto_mutual_info_stats_gaussian_fmmi(const double *x, int length, int tau) nogil 

cdef double outlier_include_np_mdrmd(const double *x, int length, int sign, double inc) nogil
...
cdef double fluct_anal_2_50_1_logi_prop_r1(const double *x, int length, int lag, int how) nogil
