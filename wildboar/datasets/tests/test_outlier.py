...
class SetupLabeler:
    ...
    def _new_data(self, n_samples, n_timestep, classes, n_classes):
    ...
    def test_binary(self):
        x, y = self._new_data(20, 10, [1, 2], [15, 5])
        self._run_labeler_suite(x, y, *(np.unique(y, return_counts=True)))

    def test_multiclass(self):
        x, y = self._new_data(20, 10, [1, 2, 3], [8, 7, 5])
        self._run_labeler_suite(x, y, *(np.unique(y, return_counts=True)))
    ...
    def _run_labeler_n_outliers(self, x, y, classes, label_count, n_outliers):
        new_x, new_x_1, new_y, new_y_1 = self._run_labeler(self._new_labeler(n_outliers), x, y)
    ...
    def _run_labeler(self, labeler, x, y):
